...
public:
    ...
    {
        //
        cout << "[EXCEPTION] No convergence" << endl;
    }
...
class OutputSample
{
    ...
    public:
        OutputSample(double v=0.0) : value(v)
        {
            //
        }
...
protected:
    ...
    VectorType x;
    VectorType x_prev;
    VectorType x_prev_prev;
    VectorType x_predict;
    VectorType u_cp;
    VectorType i_cp;
    VectorType x_plus;
    VectorType x_minus;
    ...
    void J(VectorType &x, double u_in, VectorType &u_cp, VectorType &i_cp)
    {
        ...
        for(int i = 1; i <= equation_count; ++i)
        {
            ...
            x_plus(i) += h;
            x_minus(i) -= h;
            Jacobi(_, i) = (f(x_plus, u_in, u_cp, i_cp) - f(x_minus, u_in, u_cp, i_cp))
                           / (2.0 * h);
        }
    }
...
public:
    ...
    Block(float out_div=1.0, int e_q=0) : out_divider(out_div), iterated(0), equation_count(e_q)
    {
        ...
        x_predict = VectorType(equation_count);
        lte = VectorType(equation_count);
        lte_prev = VectorType(equation_count);
        u_cp = VectorType(2);
        i_cp = VectorType(2);
    ...
    OutputSample process(float u_in)
    {
        ...
        return OutputSample(x(4) / out_divider);
    }
...
{
    // in_file out_file f_s start_second, stop_second
    f_s = atoi(argv[3]);
    ...
    SecondBlock block2 = SecondBlock(10.0);
    SecondBlock block3 = SecondBlock(10.0);
    SecondBlock block4 = SecondBlock(150.0);
    ...
    {
        in_file.seek(start_seconds * f_s + sample, SEEK_SET);
        ...
        try
        {
            ...
            {
                out = block->process(u_processed);
                u_processed = out.getValue();
            }
            u_out = u_processed;
        }
